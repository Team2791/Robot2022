...
public final class Constants {
    ...
    // PID CONSTANTS
    // feedforward (ff): reduces error in PID system (prediction technique);
    // probably won't use this maybe
    // kP: increasing this increases the speed of the control system response aka
    // reduces oscillations
    // kP values that are too high will result in more oscillations
    // kD: increasing this will help decrease the amplitude of the oscillations
    // (prevents overshooting)
    // to determine values, start with kP; increase until oscillations start, then
    // decrease back down to where there were no oscillations
    ...
    public static final double kUpperBackCloseSpeed = -.39; // -.38 practice day orig //.33 and .38 sometimes hitting
                                                            // lip

    public static final double kLowerFrontCloseSpeed = .1; // 0.111;//0.1; //.07 with firm balls was backdriving the
                                                           // motor.
